...
mc <- mclogit(cbind(case, strata) ~ ruggedness + landcover + disthighway, random=~1|id, data = cscR.all)

summary(mc) 

mc_quad <-  mclogit(cbind(case, strata) ~ landcover + ruggedness + I(ruggedness^2)  + disthighway + I(disthighway^2), random=~1|id, data = cscR.all)
summary(mc_quad) 
