...
@
\noindent The result of this code chunk is shown in Figure~\ref{fig:time_seq}. 
...
The next step would be to round time values which slightly differ from the desired one using the function \texttt{sett0}. This however should NOT be used to convert a completely irregular trajectory into a regular one but compensate for minor descrepancies. 
...
\noindent Just to get a better overview we created a nice plot by using the following function.
<<>>=
...
<<pairs, echo=TRUE, dev='pdf', fig.show='hide'>>=
...
\captionsetup{width=0.8\textwidth}
\centering
\includegraphics[width=1\textwidth]{pairs.pdf}
...
If you want to plot all effects:
...
\captionsetup{width=0.6\textwidth}
\centering
\includegraphics[width=0.6\textwidth]{model_1.pdf}
...
\caption{Prediction from the GLMM.}
\label{fig:pred_1}
\end{figure}
...
\caption{Prediction from the MCLOGIT.}
\label{fig:pred_2}
\end{figure}
