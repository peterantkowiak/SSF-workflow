...
\author{Peter Antkowiak\thanks{M.Sc. programme ``GIS und Umweltmodellierung'' at University of Freiburg} \and Henriette Tripke\thanks{M.Sc. programme ``Wildlife, Biodiversity and Vegetation'' at University of Freiburg} \and Carolin Wilhelm\thanks{M.Sc. programme ``Wildlife, Biodiversity and Vegetation'' at University of Freiburg}}
...
\noindent In addition to Resources Selection Functions (RSF) another powerful tool for evaluating data on animal movements and habitat selection are Step Selection Functions (SSF). The latter are used to estimate resource selection by comparing observed habitat use with available structures. Given GPS locations of a collared individual are connected by a linear segment. These segments are considered as steps. The fix rate of the GPS locations that influence the step length should be choosen carefully (i.e. by conducting a pilot study) to meet the requirements of the study questions, the target species and its behaviour. Then random steps are calculated by taking measured angle and distance along steps and using the observed positions as starting points. These alternative steps represent the available habitat, which could have been chosen, within a realistic step length of the observed positions. Finally, we can compare spatial attributes on  both and test for effects that explain habitat selection by animals \cite{thurfjell2014applications}. For an easier understanding of the terminology we use, we have created Figure~\ref{fig:Burst}  which explains the most important terms.\\ 

\begin{figure} [!htbp]
\captionsetup{width=0.8\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{explain_a_burst.png} 
\caption{Animal movements from A to B. We will use the depicted terms throughout the tutorial.}
\label{fig:Burst}
\end{figure}


So far, SSF models were mainly done using Geospatial Modelling Environment (GME) that only works with a GIS software\footnote{www.spatialecology.com/gme/}. However, many packages for analyzing animal movements are provided in R. None of these packages is designed for doing a SSF only or is at least missing fundamental steps. For example, the \texttt{adehabitatLT} has many features for analyzing your telemetry data (We highly recommend reading the help file from 2011 \footnote{http://cran.r-project.org/web/packages/adehabitatLT/vignettes/adehabitatLT.pdf}) but is missing any function to generate random steps. Another problem we faced when exploring the available packages in R was that some of them are lacking detailed descriptions on how to use their examples. The data for these examples is well prepared but for a user new to using SSFs, it is hard to understand the structure of these data.\\ The aim of this tutorial is to collect all the functions necessary to conduct a SSF and order and describe them in a  way that intuitively makes you understand how to run a SSF with your own data. Each step will be explained using an exemplary dataset of GPS locations collected from seven Cougars (\textit{Puma concolor}) in the year 2010 (in the following adressed as ``xmpl''). Since the study design is fundemental to successfully implement SSFs, we recommend to read  Thurfjell et al. (2014) \cite{thurfjell2014applications}, who provide different options and suggestions for conducting SSFs, before starting your own study.\\
...
\noindent Before you can actually start using this tutorial for conducting SSF you need to install a bunch of packages in R. (For faster processing the \texttt{install.packages} function is deactivated in the source file.) \\
...
<<>>=
setwd("/home/Peter/Dokumente/uni/WS_14_15/Best Practice R/SSF-Workflow/Report/")
...
\noindent To work with one of the tables only you can use \texttt{[ ]}. This is still an ``ltraj'' object. For example:
<<>>=
xmpl.ltr[2]
@

\noindent If you want to have a look at all the data stored for one single individual, or ``use'' it as a data frame you can use \texttt{[[ ]]}. If you assign it to a name, it is automatically converted and ready to use:
<<>>=
xmpl.df.10286 = xmpl.ltr[[1]]
@


\noindent To get a visual impression of your data you can plot the trajectory for all or for one particular animal (Figure~\ref{fig:ltraj_plot}):
...
\noindent You can then look at the trajectory of a single individual only (Figure~\ref{fig:ind_plot}).
...
\noindent In Figure~\ref{fig:time_seq}, the relocations that are 3 hours apart, are plottet at value 1 on the y-axis, if one relocation is missing (time lag of 6 hours) the relocation gets the value 2. As you can see, there are a lot of relocations missing. To keep the time lag constant, we now want to split the existing bursts (individuals) into ``sub-bursts'' where the time lag is bigger than 3 hours. To cut our data at our desired interval, we need a function which defines \texttt{dt}. Because we want to keep relocations which are only a few minutes ``wrong'', we added 10 extra minutes.\\
...
\noindent Note: We still keep data with different time lags (but not more than 3:10 h). As this is an irregular type II trajectory, it might cause problems with different functions (e.g. checking for autocorrelation). A constant time lag is necessary to have comparable trajectories for further analyses. There are functions to convert it into a regular one, look at the \texttt{adehabitatLT} tutorial by C. Calenge \cite{0.3.162014}, the help section to get more information or define an explicit time lag in the function above. 
A nice way to take care of our problem with minor delays of the relocations, which might be caused by the time the GPS needs to relocate, is to round. You can first look for missing time values and replace them via setting a reference date. After that you can round time values which slightly differ from the desired one using the function \texttt{sett0}. This however should NOT be used to convert a completely irregular trajectory into a regular one but compensate for minor descrepancies. Detailed instructions on how to do that can also be found in the totorial by Calenge. \cite{0.3.162014}\\


\noindent In general there are two options of cutting the trajectory depending on nextr. If it is set as FALSE, the burst stops before the first relocation matching the criterion. if it is set as TRUE, it stops after. \cite{0.3.162014}

\subsection{``ltraj'' diagnostics}

After defining your `` bursts'', there are already some options to check your created ``ltraj'' object. Those are described in the tutorial ``Analysis of Animal Movements in R: the \texttt{adehabitatLT} Package'' by clement Calenge \cite{Package2011}. As our dataset was revised already and missing values have been removed, we decided not to interpolate, we seperated all sequences according to the time lag. We therefore only give suggestions what could have been done additionaly but might result in a better output depending on your research question.

\subsubsection{Viewing missing values}
\noindent Depending on the wheather, the location of the animal or technical reasons, there might be missing relocations in your dataset. A first option is therefore to check for your ``ltraj'' object for missing values. A simple function helps you here:
...
\noindent Figure~\ref{fig:diag_1} shows where your dataset has missing values but as our data does not include any NAs, there is only one straight line. \\


\subsubsection{Replace and round time}

A first step for interpolating your dataset would be replacing the NAs in your \texttt{date} column by creating new ones with the \texttt{setNA} function. For that, you need to define a reference date. By doing that, you create a lot of ``relocations'' with NAs in the coordinates column.

\noindent The next step would be to round time values which slightly differ from the desired one using the function \texttt{sett0}. This however should NOT be used to convert a completely irregular trajectory into a regular one but compensate for minor descrepancies. 


\subsubsection{Testing for Autocorrelation}%

\noindent To ``understand'' the movement patterns of an animal, is it essential to check the parameters stored in the trajectory (dist, dx, dy, angle) for autocorrelation. A positive autocorrelation means, that values closer to each other tend to be more similar.

\noindent To test for autocorrelation, the ltraj object needs to be of type I or consist of constant time lags. As our time lags are not constant, we transform our trajectory into type I by using a simple \texttt{typeII2typeI} function.

<<>>=
xmpl.ltr.t1 <- typeII2typeI(xmpl.cut)
@

\noindent The new format does not include any information concerning time and date:
<<>>=
head(xmpl.ltr.t1)
@


\noindent To test the three linear parameters (\texttt{dist, dx, dy}), the independence test of Wald and Wolfowitz (1994) can be used. It tests the sequential autocorrelation in a vector. It can be implemented as \texttt{wawotest.ltraj} for each burst in a \texttt{ltraj} object. This function removes all NAs before running the test \cite{Package2011}.


<<eval=FALSE>>=
wawotest(xmpl.ltr.t1) 
@
\noindent The functions returns parameters to test for correlation for every burst (827 in our case!, this is why we did not let it run). The p-Values indicate the correlation but as our time lag is not constant, the interpretation does not make much sense. 

\noindent To identify at which scales autocorrelation occurs, an autocorrelation function (ACF) can be used. Here, the ltraj object needs to be regular. Similar tests can be done for angular parameters. For further information check out the tutorial by Clement Calenge \cite{Package2011}.


\subsubsection{Distinguish different behaviors}
It is possible to divide a trajectory into segments characterized by a homogenous behaviour. You can either use a ``Broken Stick Model'', the method of ``Gueguen'' or the method of ``Lavielle'' \cite{Package2011}. 
...
\noindent To generate random steps for the example dataset run the \texttt{rdSteps} function as follows:
...
When looking at the first column of the table you will notice that it only starts with the second point (10286.2). The first and the last relocations contribute to the calculation but do not have their ``own'' random steps. For the first point there is no previous step a turning angle could be calculated from, for the last point, no following endpoint is known. As a consequence, the number of points with random steps is lower than your actual recorded relocations.
Furthermore, this means that each burst must at least contain three observed positions to create random steps for one relocation.\\ 
...
ruggedness <- raster("/home/Peter/Dokumente/uni/WS_14_15/Best Practice R/Dataset/NEW GIS LAYERS/tri1") 
#/NEW GIS LAYERS/tri1
landcover <- raster("/home/Peter/Dokumente/uni/WS_14_15/Best Practice R/Dataset/NEW GIS LAYERS/lc_30")
#/NEW GIS LAYERS/lc_30
canopycover <- raster("/home/Peter/Dokumente/uni/WS_14_15/Best Practice R/Dataset/NEW GIS LAYERS/cc_abmt")
#/NEW GIS LAYERS/cc_abmt
disthighway <- raster("/home/Peter/Dokumente/uni/WS_14_15/Best Practice R/Dataset/NEW GIS LAYERS/disthwy")
#/NEW GIS LAYERS/disthwy
distroad <- raster("/home/Peter/Dokumente/uni/WS_14_15/Best Practice R/Dataset/NEW GIS LAYERS/distsmrd")
#/NEW GIS LAYERS/distsmrd
...
\noindent Just to get a better overview we created a nice plot (Figure~\ref{fig:pairs}) by using the following function.
<<>>=
...
<<pairs, echo=TRUE, dev='png', fig.show='hide'>>=
...
\caption{Beautiful plot to examine correlations and the structure of each parameter. The lower panel shows the scatter plots for each combination, the upper panel provides the correlation coefficients and the diagonal panel shows a histogram for each parameter.}
\label{fig:pairs}
\end{figure}
...
There are several options to analyze the generated data. As Thurfjell et al. 2014 \cite{thurfjell2014applications} describe in their review of SSFs, conditional logistic regression has been the most commonly used procedure. Recently, researchers have tried to account for among-individual heterogeneity in their dataset. Thurfjell et al. 2014 \cite{thurfjell2014applications} recommend two R packages providing the necessary functionality: The \texttt{lme4} package and the \texttt{mclogit} package. For giving two simple examples, we will demonstrate data analysis with a mixed conditional logistic regression \texttt{(mclogit)} and a generalized linear mixed model \texttt{(lme4)}. 
...
\noindent Both models should produce rather similar outputs. Certainly, you are free to implement any type of model you find more elaborate or more suitable.

\noindent In a second step, we will use one of the fitted models to generate predictions and plot them.


\subsection{Generalized Linear Mixed Model}%------------------------------
Generalized Linear Mixed Models are rather complex. As they are not the main subject of this tutorial, we will not go into detail here. 
For analyzing our dataset, you need to use the \texttt{glmer} function from the ``lme4'' package. Your error distribution needs to be binomial and you should include nested random effects for ID and strata to get the right structure of pseudo replication. The code looks as follows: 
...
\noindent The model does not run and we get error messages suggesting us to scale the variables. 
...
Integrated into the code, the scaled model looks like this:
...
The model output indicates significant effects for land cover and ruggedness of terrain.
If you want to plot all effects (Figure~\ref{fig:model_1}):

<<model_1, echo=TRUE, dev="pdf", fig.show="hide">>=
...
\subsection{Predictions for GLMM}
After having fitted the models, you can now calculate and plot some predictions.
With the regression coefficients (beta) and the input data (x), you can calculate predictions using the following formula:

\[
  \displaystyle w(x) = exp(\beta 1 * x1 + \beta 2 * x2 + ... + \beta p * xp)
  \]

In our regression models, we found significance for landcover and ruggedness. For this reason we will use them here and calculate predictions for ruggedness, differentiated by landcover = forest vs. open. 
To do so, we first need to fix the other parameters. In the model equation, we set the distance to road and its squared effect to their median. To receive predictions for landcover = forest, the ``landcoveropen'' estimate is left out in the first place.
As ruggedness is the parameter to estimate, it should vary along a vector corresponding to the range of real ruggedness levels. Predicting for all observed ruggedness values would take too much time given our 54,351 data points. Instead we look up minimum and maximum values of the scaled ruggedness values and generate a corresponding but smaller vector.

<<>>=
min(scale(xmpl.steps.df$ruggedness))
max(scale(xmpl.steps.df$ruggedness))
@

\noindent Based on these values, generate the following data frame:
<<>>=
GLMMpreds = data.frame(ruggedness=seq(-1.5,15,0.1))
@

\noindent Calculating the predictions for selection in relation to ruggedness in the forest:
<<>>=
GLMMpreds$wruggforest <- exp(              
        xmpl.glmm.fit.sc@beta[3] * GLMMpreds$ruggedness  + 
        xmpl.glmm.fit.sc@beta[4] * GLMMpreds$ruggedness^2 + 
        xmpl.glmm.fit.sc@beta[5] * median(scale(xmpl.steps.df$distroad)) + 
        xmpl.glmm.fit.sc@beta[6] * median((scale(xmpl.steps.df$distroad))^2))
@

\noindent To calculate the predictions for selection in relation to ruggedness in the open, you need to include the corresponding estimate (beta 2) in the equation:
<<>>=
GLMMpreds$wruggopen <- exp(
...
\noindent Before plotting the predictions, you need to rescale the x-vector. You can do so by multiplying with the standard deviation of the orignial ruggedness values and adding their mean:
<<>>=
mean(xmpl.steps.df$ruggedness)
sd(xmpl.steps.df$ruggedness)

GLMMpreds$ruggednessResc <- (GLMMpreds$ruggedness * sd(xmpl.steps.df$ruggedness) + mean(xmpl.steps.df$ruggedness))

@

\noindent Finally, you are ready to plot the predictions (Figure~\ref{fig:pred_1}):
<<pred_1, echo=TRUE, dev="pdf", fig.show="hide">>=

plot(GLMMpreds$ruggednessResc, GLMMpreds$wruggforest, 
     type="l", col="darkgreen", lwd=2, ylim=c(0,1), xlim=c(0,250), 
     xlab="ruggedness", ylab="W", las=1)
lines(GLMMpreds$ruggednessResc, GLMMpreds$wruggopen, type="l", col="green", lwd=2)
legend("topright",legend=c("forest","open"), col=c("darkgreen","green"), lwd=2)
...
\captionsetup{width=0.6\textwidth}
\centering
\includegraphics[width=0.6\textwidth]{pred_1.pdf}
\caption{GLMM predictions for selection in relation to ruggedness.}
\label{fig:pred_1}
\end{figure}

\noindent The plot nicely shows that cougars prefer flat terrain and avoid very rugged areas. Furthermore they prefer forest-covered areas over open areas.


%-----------------------------------------
\subsection{Predictions for MCLOGIT}

\noindent Predicting values with the MCLOGIT model works pretty similar to the GLMM. Looking up minimum and maximum values, now for the unscaled ruggedness. Then generating the data frame:
<<>>=

min(xmpl.steps.df$ruggedness)
max(xmpl.steps.df$ruggedness)

MCLOGpreds = data.frame(ruggedness=seq(0,250,1))
@

\noindent Calculating predictions for both forested and open areas:
<<>>=
MCLOGpreds$wruggforest = exp(
        xmpl.logit.fit$coefficients[2] * MCLOGpreds$ruggedness  + 
        xmpl.logit.fit$coefficients[3] * MCLOGpreds$ruggedness^2 + 
        xmpl.logit.fit$coefficients[4] * median(xmpl.steps.df$distroad) +
        xmpl.logit.fit$coefficients[5] * (median(xmpl.steps.df$distroad))^2)

MCLOGpreds$wruggopen = exp(
        xmpl.logit.fit$coefficients[1] +
        xmpl.logit.fit$coefficients[2] * MCLOGpreds$ruggedness  + 
        xmpl.logit.fit$coefficients[3] * MCLOGpreds$ruggedness^2 + 
        xmpl.logit.fit$coefficients[4] * median(xmpl.steps.df$distroad) +
        xmpl.logit.fit$coefficients[5] * (median(xmpl.steps.df$distroad))^2)
...
\noindent As the data were not scaled in the MCLOGIT model, no rescaling is necessary and you can proceed to plotting (Figure~\ref{fig:pred_2}) right away:
<<pred_2, echo=TRUE, dev="pdf", fig.show="hide">>=
plot(MCLOGpreds$ruggedness,MCLOGpreds$wruggforest, 
     type="l", col="darkgreen", lwd=2, ylim=c(0,1), 
     xlab="ruggedness", ylab="W", las=1)
lines(MCLOGpreds$ruggedness,MCLOGpreds$wruggopen,type="l", col="green", lwd=2)
legend("topright",legend=c("forest","open"), col=c("darkgreen","green"), lwd=2)
@
...
\captionsetup{width=0.6\textwidth}
\centering
\includegraphics[width=0.6\textwidth]{pred_2.pdf}
\caption{MCLOGIT predictions for selection in relation to ruggedness.}
\label{fig:pred_2}
\end{figure}

\noindent The plot of the MCLOGIT model looks pretty similar to the GLMM. That is what we hoped for.

\noindent We hope you had some fun with our tutorial and got some new ideas to develop further!


\section{Acknowledgements}
Many thanks to the TeX and R communities \cite{RCoreTeam2014} and all the helpful online tutorial writers!
We explicitly want to thank Mathieu Basille for the useful \texttt{rdSteps} function from his ``hab'' package \footnote{https://github.com/basille/hab} that is not in the official R repositories any more.

Special thanks goes to our best team mate Simone \ding{170} \ding{96}!


\newpage
\section{Appendix}

For reproducibility, our R Session Info:
<<echo=FALSE>>=
sessionInfo()
@
