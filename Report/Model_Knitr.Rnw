...
\[
\displaystyle mclogit(case, strata) ~ 
ruggedness + ruggedness^2 + disthighway + disthighway^2 + distroad + distroad^2 + landcover, random=~1|id
\]
...
xmpl.glmm.fit <- glmer(case ~ 
    ...
    scale(canopycover) + I((scale(canopycover))^2) +
...
summary(xmpl.glmm.fit)
                      ...
                      ruggedness + landcover + canopycover + disthighway + distroad + (1|id/strata), family = binomial, data=cscR.all)
