...
library(effects)
plot(allEffects(xmpl.logit.fit)) # how to plot??

@



The mixed conditional logistic regression extends the usually unconditional logistic regression and adapts it for matched observations.


conditional logistic regression - special case of (usually unconditional) logistic regression for matched observations

strata + random effect for individual


without mixed: pseudo-replication because replicates are not independent but rather linked e.g. by animal.
...
mydata$wrugg = exp(
              xmpl.glmm.fit.sc@beta[1] +
              xmpl.glmm.fit.sc@beta[4] * mydata$ruggedness  + 
              xmpl.glmm.fit.sc@beta[5] * mydata$ruggedness^2 + 
              xmpl.glmm.fit.sc@beta[6] * median(xmpl.steps.df$distroad) + 
              xmpl.glmm.fit.sc@beta[7] * (median(xmpl.steps.df$distroad))^2)

plot(mydata$ruggedness, mydata$wrugg, type="l", col="darkgreen", lwd=2)

mydata$wrugg = exp(mc_quad$coefficients[1] + mc_quad$coefficients[3] +
                      mc_quad$coefficients[2] * mydata$ruggedness  + 
                      mc_quad$coefficients[3] * mydata$ruggedness^2 + 
                      mc_quad$coefficients[4] * median(cscR.all$distroad) +
                      mc_quad$coefficients[5] * (median(cscR.all$distroad))^2)
