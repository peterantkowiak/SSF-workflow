...
Both coordinates and animal id can easily be acquired from the Spatial Points Data Frame. The timestamp however, is not stored in the required format yet and therefore

# And finally extract date and time:
date <- as.POSIXct(strptime(paste(xmpl.df$LMT_DATE, xmpl.df$LMT_TIME), "%d/%m/%Y  %H:%M:%S"))
@
...
cougarsLTR <- hab:::as.ltraj(xmpl.spdf@coords, date, id = xmpl.spdf$cat) 
