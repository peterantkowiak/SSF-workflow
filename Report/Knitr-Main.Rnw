...
\centering
\includegraphics[width=0.6\textwidth]{ltraj_plot.pdf}
...
\captionsetup{width=0.6\textwidth}
\centering
\includegraphics[width=0.6\textwidth]{time_seq.pdf}
...
<<corr_1, echo=TRUE, dev="pdf", fig.show="hide">>=
...
ruggedness <- raster("P:/SSF PROJECT/
                     NEW GIS LAYERS/tri1") 
landcover <- raster("P:/SSF PROJECT/
                    NEW GIS LAYERS/lc_30") 
canopycover <- raster("P:/SSF PROJECT/
                      NEW GIS LAYERS/cc_abmt") 
disthighway <- raster("P:/SSF PROJECT/
                      NEW GIS LAYERS/disthwy")
distroad <- raster("P:/SSF PROJECT/
                   NEW GIS LAYERS/distsmrd")

# It is enough to load the whole folder were your *.adf files are stored in. The function raster() finds the needed files. 
@
